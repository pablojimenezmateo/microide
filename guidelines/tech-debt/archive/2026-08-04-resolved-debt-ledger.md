# Resolved tech debt — ledger split of 2026-08-04

`dev-docs/project/known-tech-debt.md` declares itself the queue for debt that is
"open, actionable, and still present in the tree", and says closed debt does not
live there. It had drifted badly from that: 555 of 637 entries were resolved or
won't-do, and the open work was buried among them.

This file is what was removed. It is a record, not a queue — nothing here needs
doing. The split was mechanical and verified: an entry moved if its own first line
carried a RESOLVED / FIXED / WON'T-DO marker, or if its section title declared the
whole section closed. 82 entries stayed, 555 moved, none were lost or duplicated.

Section headings are reproduced so each entry keeps the context it was filed under.
Deeper history (the per-change proposals and specs) lives in
`openspec/changes/archive/` and in the commits named by each entry.
