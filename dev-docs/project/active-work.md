...
- tab/project **detach and reattach** across windows. Because the shell is single-window per process, `detach-tab`/`detach-project` (tab/project context menu, or dragging a tab out of the strip) writes the tab's live state — including unsaved edits, via an ordinary `PersistedProjectSessionState` handoff file through `PersistenceService` — and launches a second microide process (`platform::SpawnDetached` + `--detach-handoff`). Dragging a tab onto another running window hands it off over the control channel (`accept-tab-handoff`), targeted by each window's live bounds published in its control descriptor (`WorkspaceDetachCoordinator`, `ResolveDropTarget`). Editor-tab detach children are session-ephemeral (never clobber the parent's saved session); project detach children own their root and persist normally. **Every** window binds a control socket at startup so it is always a reattach drop target with no opt-in; `control.enabled`/`--control` only widens that socket from handoff-only ("restricted") to the full command/query surface — the socket is never torn down mid-run (which removes the old `EnsureControlChannelForHandoff`/`--set control.enabled` handoff hacks and the `Start`/`Stop` churn that previously stranded a headless driver on a live setting change)
