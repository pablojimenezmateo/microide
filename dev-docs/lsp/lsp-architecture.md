...
| `WorkspaceLspClientInternal.h` | `LspClient::Impl` declaration: fields, nested `QueuedMessage`/`PendingRequest`, small inline accessors, and the `DispatchResultRequest` template. **Declaration-heavy** — heavy bodies live in the shared transport mixin and the TUs below. |
| `StdioJsonRpcClientTransport.h` | **Under `src/workspace/`** (shared with DAP — the second half of the extraction that started with the framer). CRTP mixin holding the whole transport half: I/O thread body (`IoMain`), outbound send/drain (`DrainOutbound`, `SendMessageImmediate`, `SendMessageBuilderAfterInitialize`), queue caps, `WaitStdoutReadable`. `LspClient::Impl` supplies the protocol hooks (`DispatchMessage`, `FailPendingRequests`, `SetLastError`, `TraceSend`, `WakeMainThread`). |
