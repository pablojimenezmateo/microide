...
- `llm`: registers stdio-backed Codex chat and inline-completion agents; by default it delegates both flows to the local `codex exec` CLI in read-only mode, so auth, provider selection, and model execution stay aligned with the real Codex toolchain; configure the model with `llm.codex.model` (default `gpt-5.4`) and the binary path with `llm.codex.binary` (default `codex`); `llm-status` and `llm-logout` use the Codex CLI when available, while `llm-login` can still seed `~/.codex/auth.json` from inside microide; override either stdio endpoint directly with `llm.chat_command` or `llm.inline_command`; disable either registration with `llm.chat_enabled` or `llm.inline_enabled`
