...
- `typescript-lsp`: registers TypeScript LSP for the `typescript` language id (`.ts` and `.tsx` via current syntax detection); resolves binary in this order: project `node_modules/.bin/typescript-language-server`, PATH/global binary, npm global bin, yarn global bin, then `npx --yes typescript-language-server --stdio`; optional project settings are `typescript_lsp.binary` and `typescript_lsp.tsserver_path`
