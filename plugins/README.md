...
- `llm`: registers stdio-backed Codex chat and inline-completion agents; by default it calls the OpenAI chat completions API through `python3`, uses `llm-login`/`llm-status`/`llm-logout` for `~/.codex/auth.json` device auth, and falls back to `OPENAI_API_KEY` when present; configure the model with user setting `llm.codex.model` (default `gpt-5.4`); override either stdio endpoint directly with `llm.chat_command` or `llm.inline_command`; disable either registration with `llm.chat_enabled` or `llm.inline_enabled`
