...
### Fixed
- Find widget: typing into the find box on a large file rescanned and case-folded every line that had matched the shorter query, on every keystroke — for a query matching most lines that is a whole cold scan per character (measured 48 ms per pass on a 50k-line file, against 16 ms before). The rescan is only needed when the previous query can occur inside itself (`aa` recurs at its own offset 1, `perfocc` never does); with no such overlap the cold scan skipped nothing, so every occurrence of the prefix was already recorded and the refine checks those columns directly. The whole-line fold buffer is also reused across keystrokes rather than allocated per call. The 3,000-round refine-vs-cold-scan enumeration covers both branches, and a new case pins them at their boundary.
- Multi-caret: every caret-set mutation ran the overlap merge, and the merge allocated — a scratch vector per call, sized by the caret set, plus a rebuild vector. A held column-select gesture or a run of Ctrl+D therefore allocated once or twice per keystroke, growing with the selection (400 allocations over a 400-line box select, 190 over 96 Ctrl+D presses). The merge now reuses one buffer that grows geometrically and writes the merged set back in place.
