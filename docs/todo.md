...
- `[~]` per-line git blame shadow text now renders only for the caret line plus one line above and below on tracked non-large on-disk files, including saved but uncommitted content, with hover commit details and SHA copy through the async viewport-scoped cache described in `docs/blame-shadow-text.md`, but broader repo validation and any policy expansion still need work
- `[-]` soft wrap
...
- `[~]` the SDL_ttf backend now relies on cached whole-string textures for correct ASCII rendering; the per-glyph ASCII shortcut was removed because it distorted editor text, and richer atlas-style batching for broader text cases is still open
- `[x]` the app now keeps a retained scene texture, and redraw ownership now lives in `WorkspaceShell` through explicit per-surface invalidations for chrome, prompts, overlays, sidebars, breadcrumbs, tab strips, editor surfaces, bottom panels, caret blink paths, and state-changing tab or sidebar transitions
- `[x]` caret-blink invalidation no longer forces a full shell redraw
- `[~]` dirty-rect ownership is now explicit and the retained-scene path supports multiple dirty rects per event; the renderer now widens those clips by a small font-aware bleed margin so caret and row-band redraws do not cache clipped glyph edges, active editor edits repaint the affected line band or line-to-bottom region in the focused pane, compare selection changes repaint the affected row bands, merge conflict selection and hover repaint the affected conflict bands, terminal typing repaints only panel content, and terminal wake updates intentionally remain bottom-panel-wide because they may update tab titles, but terminal updates and some compare or merge edit paths still redraw larger vertical regions when downstream rows shift
...
- `[x]` retained-render regression coverage now checks sidebar-mode switches and file-open tab transitions against clean full redraws so stale-scene holes do not silently regress
...
- `[~]` blame coverage now exercises the incremental parser, cache invalidation, dirty-buffer suppression, viewport-window loading, caret-local inline placement, hover popup SHA copy, and narrow-pane suppression, but broader repo-scale validation is still missing
