...
- `WorkspaceShell` still declares 17 `friend class` relationships so coordinators can reach into
...
- `WorkspaceContext` now owns project catalog, active project, prompt, menu, and interaction
  ...
  `WorkspaceDirtyPromptCoordinator` are the first exceptions: they now depend on
...
3. Migrate the least UI-coupled controllers first: project catalog, persistence, lifecycle.
   ...
   `WorkspacePersistenceCoordinator`, `WorkspaceLifecycleCoordinator`, and
   `WorkspaceDirtyPromptCoordinator` now consume `WorkspaceContext` plus explicit callbacks
