...
## 8. Syntax Highlighter Allocates Per Match on Every Rendered Line

Impact:
- High
- Directly adds latency to every frame that renders any highlighted editor content

What is wrong:
- `FindFirstRegex` and `FindAllRegex` in `RuntimeSyntaxRegistry.cpp` call
  `pattern.CreateMatchData()` on every invocation, which calls
  `pcre2_match_data_create_from_pattern` — a heap allocation per match
- `FindAllRegex` also returns a `std::vector<MatchRange>` by value, triggering an additional
  heap allocation per rule per segment per line per frame
- With ~50 visible lines and dozens of pattern rules, this is hundreds of malloc/free cycles per
  frame before any text is actually drawn

Evidence:
- `src/editor/RuntimeSyntaxRegistry.cpp:86` — `auto match_data = pattern.CreateMatchData()`
  inside `FindFirstRegex` (called per-rule per-line)
- `src/editor/RuntimeSyntaxRegistry.cpp:103–134` — `FindAllRegex` returns `vector<MatchRange>`
  by value
- `src/editor/RuntimeSyntaxRegistry.cpp:507` — `ApplyPatternRules` calls `FindAllRegex` in a
  per-rule loop

What a good fix looks like:
- Use a thread-local `RegexMatchData` per compiled pattern so match data is allocated once and
  reused across calls without locking
- Pass a reusable `std::vector<MatchRange>&` out-param into `FindAllRegex` so callers can own
  one buffer per line and avoid repeated vector allocations

## 9. Highlight Cache and Checkpoint Array Are Fully Cleared on Every Edit

Impact:
- High
- Adds render latency after every keystroke for files longer than a few hundred lines

What is wrong:
- `InvalidateDerivedCaches` (called by every edit path) clears all 256 highlight cache entries,
  all per-line syntax states, and all checkpoints unconditionally
- The next render after an edit triggers `EnsureHighlightCheckpoints`, which synchronously
  advances through every line to rebuild checkpoints before rendering the visible region
- For a 10,000-line file with `kHighlightCheckpointInterval=128`, this is ~78 `AdvanceState`
  passes — each running PCRE2 matches against a full line — before the first frame is drawn

Evidence:
- `src/editor/TextViewport.cpp:708` — `InvalidateDerivedCaches` clears everything
- `src/editor/TextViewport.cpp:660` — `EnsureHighlightCheckpoints` rebuilds all checkpoints
  synchronously on first access after any edit
- `src/editor/TextViewport.h:18` — `kHighlightCheckpointInterval = 128`

What a good fix looks like:
- Pass the edit start line into `InvalidateDerivedCaches` and only flush highlight states and
  checkpoints at or after that line; everything before is still valid
- Build checkpoints lazily: only compute the checkpoint needed to reach the visible region, not
  all checkpoints for the entire document

Likely starting files:
- `src/editor/TextViewport.cpp` — `InvalidateDerivedCaches`, `EnsureHighlightCheckpoints`,
  `ApplyHistoryEntry`

## 10. `VisibleLineCacheKey` Includes Caret Column, Causing Cache Misses on Cursor Movement

Impact:
- Medium
- Every left/right arrow keystroke causes a layout cache miss for the active line

What is wrong:
- `VisibleLineCacheKey` includes `caret_text_column` because `BuildVisibleLine` returns caret
  position alongside text layout in the same `LayoutLine` struct
- The actual text layout (text, source_columns, text_offsets) does not change when the caret
  moves horizontally; only `caret_visible` and `caret_column` differ
- Result: horizontal cursor movement causes a cache miss and a full `BuildVisibleLine` re-run
  for the active line, even though the rendered text is identical

Evidence:
- `src/editor/TextViewport.h:158–180` — `VisibleLineCacheKey` struct includes `caret_text_column`
- `src/editor/TextLayout.cpp:77–130` — `BuildVisibleLine` computes caret fields and text layout
  in one pass

What a good fix looks like:
- Separate caret computation from text layout in `BuildVisibleLine`; remove `caret_text_column`
  from the cache key and compute caret visibility at render time from the cached layout and the
  current cursor position

Likely starting files:
- `src/editor/TextLayout.h` — `LayoutLine`, `BuildVisibleLine`
- `src/editor/TextViewport.h` — `VisibleLineCacheKey`
- `src/editor/TextViewport.cpp` — `VisibleLineLayout`

## 11. Terminal `SnapshotLineRange` Deep-Copies Lines on Every Frame

Impact:
- Medium
- Adds allocation cost on every frame that renders an active terminal panel

What is wrong:
- `SnapshotLineRange` acquires the mutex and copies all visible `TerminalLine` objects on every
  frame, even when the terminal has been idle since the last render
- Each `TerminalLine` contains a `std::vector<TerminalCell>`, so for 40 visible rows at 200
  columns each this is 8,000 cell copies plus 40 vector copies per frame while idle

Evidence:
- `src/terminal/TerminalSession.cpp:705–715` — `SnapshotLineRange` always copies
- `src/workspace/WorkspaceShellRenderBottomPanel.cpp:269` — called unconditionally on every render

What a good fix looks like:
- Add a generation counter to `TerminalSession` incremented on every write; let the render path
  compare generations and skip the snapshot copy when the terminal has not changed since the
  previous frame

## 12. Output Panel Runs Full Syntax Highlighter on Every Visible Code Line Every Frame

Impact:
- Medium
- Adds significant CPU work whenever the output panel is visible with code-context snippets

What is wrong:
- `WorkspaceShellRenderBottomPanel.cpp:314` calls `editor::runtime_syntax::HighlightLine` on
  every visible code-context snippet on every frame, even when the output channel has not changed
- This runs the full PCRE2 regex highlighter per visible output line per frame

Evidence:
- `src/workspace/WorkspaceShellRenderBottomPanel.cpp:314`

What a good fix looks like:
- Cache highlighted token arrays per output channel entry; invalidate only when the channel
  appends new lines; a `std::vector<HighlightedLine>` parallel to the channel's entry list is
  enough

## Suggested Order For Later Work

1. Fix syntax highlighter per-match allocations (`CreateMatchData`, `FindAllRegex` return value).
2. Make highlight cache and checkpoint invalidation partial rather than full.
3. Remove `caret_text_column` from the visible-line cache key.
4. Keep shrinking `WorkspaceShell` and reduce friend-based coordinator access.
5. Finish the active-viewport migration so stale floating-editor paths are harder to reintroduce.
6. Build one shared selection-aware editor model for single-line shell inputs.
7. Address terminal snapshot copy and output-panel syntax caching when profiling confirms cost.
8. Revisit project-content and indexing architecture only if profiling still shows meaningful
