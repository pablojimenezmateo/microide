...
void TestArchitectureInvariants() {
  ...
  run_rule("CheckNoDebuggerDapSurface", CheckNoDebuggerDapSurface);
  run_rule("CheckTextViewportNoCombinedLayoutRevision", CheckTextViewportNoCombinedLayoutRevision);
  ...
  run_rule("CheckSdlTtfBackendNoPerGlyphLoop", CheckSdlTtfBackendNoPerGlyphLoop);
  run_rule("CheckDecoratedTextGridRendererBatchesFills", CheckDecoratedTextGridRendererBatchesFills);
  run_rule("CheckEditorViewRendererUsesScratchRows", CheckEditorViewRendererUsesScratchRows);
  run_rule("CheckApplicationCoalescesResize", CheckApplicationCoalescesResize);
  run_rule("CheckMouseWheelUsesFractionalAccumulator", CheckMouseWheelUsesFractionalAccumulator);
  run_rule("CheckBottomPanelTerminalRectCache", CheckBottomPanelTerminalRectCache);
  // 2026-05-15 perf deep-dive round 2 invariants. See
  // docs/performance-bottleneck-deep-dive-2.md.
  run_rule("CheckNoStdStoInRenderOrBuilderTus", CheckNoStdStoInRenderOrBuilderTus);
  run_rule("CheckStatusBarRefreshIsAsyncOnly", CheckStatusBarRefreshIsAsyncOnly);
  run_rule("CheckSidebarSurfaceFallbackUsesStringView", CheckSidebarSurfaceFallbackUsesStringView);
  run_rule("CheckEditorViewModelStickyAndOccurrenceAreSpans", CheckEditorViewModelStickyAndOccurrenceAreSpans);
  run_rule("CheckRenderTuDoesNotMaterializeSingleCharOrPrefixStrings",
...
void TestArchitectureInvariantTargetedScannerFixtures() {
  ...
  Expect(!CheckNoDebuggerDapSurface(root).violations.empty(),
