...
bool TabCoordinator::OverwriteEditorTabsForPath(const std::filesystem::path& path) {
  ...
  for (auto& tab : state_.open_tabs) {
    if (tab.kind != TabEntry::Kind::Editor || !tab.editor_state.has_value()) {
      continue;
    }
    for (auto& view : tab.editor_state->views) {
      editor::TextViewport& viewport = view.viewport;
...
bool TabCoordinator::DiskSignatureMatchesOpenView(const std::filesystem::path& path,
  ...
  for (const auto& tab : state_.open_tabs) {
    if (tab.kind != TabEntry::Kind::Editor || !tab.editor_state.has_value()) {
      continue;
    }
    for (const auto& view : tab.editor_state->views) {
      if (view.viewport.path().lexically_normal() != normalized_path) {
        continue;
      }
      matched_any_view = true;
      if (!signature.SameContentAs(view.viewport.disk_signature())) {
        return false;
      }
    }
  }
