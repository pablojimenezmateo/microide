...
WorkspaceShell::SingleLineViewMetrics WorkspaceShell::ComputeSingleLineViewMetrics(
  ...
  for (std::size_t pos = 0; pos < cursor_byte;) {
    ...
    before_cursor.push_back({pos, text_renderer_.MeasureWidth(segment_view(pos, next))});
    pos = next;
  }
  ...
  while (view_end < total_len) {
    ...
    const float char_w = text_renderer_.MeasureWidth(segment_view(view_end, next));
