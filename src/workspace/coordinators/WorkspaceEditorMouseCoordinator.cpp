...
void RecordSelectionSeed(InteractionState& interaction_state,
...
void ExtendSelectionByGranularity(const InteractionState& interaction_state,
                                  editor::TextViewport& viewport,
...
bool EditorMouseCoordinator::HandleButtonDown(const SDL_Event& event,
  ...
  interaction_state_.selection_granularity = InteractionState::SelectionGranularity::Character;
  if (!shift_alt_box) {
    if (event.button.clicks == 2) {
      viewport->SelectWordAtCursor();
      RecordSelectionSeed(interaction_state_,
                          InteractionState::SelectionGranularity::Word, *viewport);
    } else if (event.button.clicks >= 3) {
      viewport->SelectLineAtCursor();
      RecordSelectionSeed(interaction_state_,
                          InteractionState::SelectionGranularity::Line, *viewport);
    }
  }
...
bool EditorMouseCoordinator::HandleSelectionMotion(const SDL_Event& event,
  ...
  if (interaction_state_.selection_granularity !=
          InteractionState::SelectionGranularity::Character &&
      !interaction_state_.editor_box_selecting) {
    ExtendSelectionByGranularity(interaction_state_, *viewport,
                                 editor::TextPosition{hit.line, hit.column});
  } else if (interaction_state_.editor_box_selecting) {
