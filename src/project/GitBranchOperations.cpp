...
GitBranchListing ListGitBranches(const std::filesystem::path& repository_root) {
  ...
  // `for-each-ref` with an explicit format is the machine-readable form; `git
  // branch` output is decorated for humans (a leading "* ", padding, and a
  // "(HEAD detached at …)" pseudo-entry) and would need to be un-decorated here.
  // Emit the FULL refname alongside the short one: a local branch may itself be
  // named "feature/topic", so a '/' in the short name says nothing about whether
  // the ref is local or remote. Only the refs/heads vs refs/remotes prefix does.
  const GitRepository::CommandResult result = repo.Execute(std::vector<std::string>{
      "for-each-ref", "--format=%(refname)%09%(refname:short)%09%(HEAD)", "refs/heads",
      "refs/remotes"});
  if (!result.success()) {
    return listing;
  }
  ...
  const std::string_view text(result.output);
  while (line_start < text.size()) {
    ...
    const std::string_view full_name = line.substr(0, first_tab);
    const std::string_view rest = line.substr(first_tab + 1);
    const std::size_t second_tab = rest.find('\t');
    const std::string_view name = second_tab == std::string_view::npos ? rest
                                                                      : rest.substr(0, second_tab);
    const bool is_head = second_tab != std::string_view::npos && rest.substr(second_tab + 1) == "*";
    if (name.empty()) {
      continue;
    }
    // `refs/remotes/origin/HEAD` is a symbolic alias, not a checkout target.
    if (full_name.ends_with("/HEAD")) {
      continue;
    }
    if (full_name.starts_with("refs/heads/")) {
      listing.local.emplace_back(name);
      if (is_head) {
        listing.current.assign(name);
      }
    } else if (full_name.starts_with("refs/remotes/")) {
      listing.remote.emplace_back(name);
    }
  }
