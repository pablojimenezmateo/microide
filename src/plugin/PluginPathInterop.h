...
// Empty roots are skipped. A two-tier check keeps the common case cheap: a lexical test
// rejects `..` escapes first (syscall-free), then weakly_canonical resolves symlinks in the
// path's existing prefix — including for not-yet-created leaves — so a missing target whose
// parent is a symlink out of the project is rejected too.
