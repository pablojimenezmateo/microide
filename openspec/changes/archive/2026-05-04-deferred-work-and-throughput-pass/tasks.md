...
- [x] 5.3 Add `visible_line_range` (start line, end line, viewport) POD field to `FrameToken`; populate it once from `EditorTabService::ActiveViewport()` in `PrepareFrameOnce`; set to sentinel when no editor tab is active
- [x] 5.4 Update all render phases that currently call `EditorTabService::ActiveViewport()` independently to read `visible_line_range` from the `FrameToken` instead; remove the now-redundant viewport calls
- [x] 5.5 Add `TabStripGeometryCache` struct (tab widths, positions, overflow offset) keyed by `{tab_count, window_width, active_tab_index}`; populate on first use and after any key change; store alongside the tab-strip render state
- [x] 5.6 Add ASAN-mode assertion in `Application::WorkspaceRenderClip` that `PrepareFrameOnce` has been called in the current frame (guards against layout-not-computed bugs at startup and after window show/hide)
