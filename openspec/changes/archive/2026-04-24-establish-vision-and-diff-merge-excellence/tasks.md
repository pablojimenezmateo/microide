...
- [x] 2.1 Update `docs/implementation-guide.md` to remove "AI/chat surfaces" from the explicit non-goals list and add a "See `openspec/specs/ai-workflows`" pointer in its place
- [x] 2.2 Update `docs/implementation-guide.md` "Durable Product Decisions" section to reference `openspec/specs/product-vision` as the authoritative product-thesis source
- [x] 2.3 Update `docs/implementation-guide.md` "Explicit Non-Goals" so it no longer contradicts `product-vision` (keep debugger-beyond-first-pass, plugin marketplaces, cloud/collaboration, recent-surfaces, soft wrap, native OS menus)
- [x] 2.4 Update `ROADMAP.md` "Medium-Term Phases > Diff and merge rewrite" section to cite `openspec/specs/diff-merge-editor` as the durable contract and `docs/diff-editor-merge-rewrite-plan.md` as the implementation plan
...
- [x] 5.1 Audit `docs/chat-pane-plan.md`: the chat pane is shipped per `docs/active-work.md` Phase 5. Move it to `docs/archive/chat-pane-plan.md` (create the directory) or delete it if nothing references it
...
- [x] 5.3 Audit `docs/text-surface-unification.md` and `docs/production-tech-debt-review.md`: confirm they still describe active guidance; archive if superseded by `guidelines/` or by the new specs
- [x] 5.4 Keep in place as durable reference: `docs/implementation-guide.md` (post-refresh), `docs/active-work.md`, `docs/known-tech-debt.md`, `docs/macos-support-plan.md`, `docs/performance-findings.md`, `docs/plugin-runtime-research.md`, `docs/runtime-profiling.md`, `docs/startup-tracing.md`
- [x] 5.5 For each archived doc, add a one-line entry in `ROADMAP.md` or `docs/active-work.md` recording "shipped; plan archived at docs/archive/..." so the history is discoverable
...
- [x] 6.4 Verify that `microide_diff_bench`, `microide_search_bench`, `MICROIDE_PERF_TRACE`, and `MICROIDE_TRACE_REDRAW` references in `README.md` and `docs/runtime-profiling.md` remain accurate after the alignment pass
- [x] 6.5 Confirm no source files under `src/` are modified by this change (specs + docs only)
- [x] 6.6 Confirm `AGENTS.md`, `CLAUDE.md`, `README.md`, `ROADMAP.md`, and `docs/implementation-guide.md` do not contradict each other on (a) AI scope, (b) diff/merge priority, (c) plugin-phase framing (shipped vs active)
...
- [x] 7.2 Confirm `docs/diff-editor-merge-rewrite-plan.md` exists; if it does not, note its absence as an open follow-up so the diff-merge implementation change has a design landing place
