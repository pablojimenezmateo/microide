...
- [Risk] Fuzzing finds long-tail bugs that aren't worth fixing immediately → Mitigation: fuzz time is bounded per CI run (60 s on PR, longer nightly); crashes go into a triage queue tracked in `docs/known-tech-debt.md`, not as merge blockers, unless they are reachable from real input.
