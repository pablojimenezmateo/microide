...
- [x] 2.13 Run the harness in `--update-baseline` mode on `perf-runner-v1` to seed every committed baseline. Flip the gate to active.
...
- [x] 6.1 Add the migration adapters on `editor::SingleLineEditor` (`SetText(std::string)`, `Snapshot()`, `Append(std::string)`) needed to round-trip the legacy state cleanly.
- [x] 6.2 Migrate every workspace state model that currently holds `util::SingleLineTextState` to hold `editor::SingleLineEditor` directly. Update view models in lockstep.
- [x] 6.3 Migrate every render call site that reads `const util::SingleLineTextState*` to take `const editor::SingleLineEditor*`. Update `RenderViewModelBuilder` accordingly.
- [x] 6.4 Delete `src/util/SingleLineText.h`, `src/util/SingleLineText.cpp`, and every `util::Set/Get/Insert*` free helper. Confirm zero references via `grep`.
- [x] 6.5 Run the harness scenarios that exercise the migrated surfaces (`typing_small_file`, `typing_large_file`, `project_search_literal`, `chat_pane_long_transcript`, `multi_tab_cycle`). Confirm green; only update baselines if intentional.
...
- [x] 8.1 Decompose `WorkspaceTabCoordinator.cpp` (1364 lines) into focused units (e.g. lifecycle, save/restore, retarget). Each ≤ 800 lines. Re-run harness `multi_tab_cycle`, `cold_startup_*`. Update lint scope.
- [x] 8.2 Decompose `WorkspaceActionServices.cpp` (1086 lines) along the existing per-domain executor seams. Re-run relevant scenarios (typing, project switch, sidebar). Update lint scope.
- [x] 8.3 Decompose `WorkspaceChatTranscript.cpp` (1170 lines) into rendering, persistence, and tool-event handling units. Re-run `chat_pane_long_transcript`. Update lint scope.
- [x] 8.4 Decompose `WorkspaceLspClient.cpp` (1126 lines) along request-type seams. Re-run scenarios that touch LSP (`linter_on_save` and any added LSP-driven scenario). Update lint scope.
- [x] 8.5 Decompose `WorkspacePersistenceBinaryFormat.cpp` (1119 lines) by record type. Re-run `cold_startup_*`. Update lint scope.
- [x] 8.6 Decompose `WorkspaceLayout.cpp` (995 lines) into per-pane geometry helpers. Re-run scroll and resize scenarios. Update lint scope.
- [x] 8.7 Decompose `WorkspaceKeyInputCoordinatorSurfaces.cpp` (735 lines, soft-cap) along surface-domain seams. Re-run typing scenarios. Update lint scope.
- [x] 8.8 Flip the coordinator-TU-size lint rule from soft-fail to hard-fail once every unit is ≤ 800 lines.

## 9. WorkspaceShellTestAccess Trim

- [x] 9.1 Audit `src/workspace/WorkspaceShellTestAccess.h` (~1500 lines). Categorize each method as: (a) duplicates a service-public API, (b) exposes test-only state genuinely unavailable through services, (c) obsolete.
- [x] 9.2 Replace category-(a) call sites in fixtures with direct service calls; delete the methods.
- [x] 9.3 Remove category-(c) methods.
- [x] 9.4 Cap the remaining file at ≤ 600 lines. Add a lint rule for the cap.
- [x] 9.5 Run the full `microide_tests` suite. Confirm green.
...
- [x] 11.1 Author scenario `long_soak_8h` reusing the idle harness driver, parsing RSS at start/midpoint/end and CPU continuously, asserting per-hour wake-up budget.
- [x] 11.2 Add a nightly CI job invoking only `long_soak_8h`. Configure thresholds.
- [x] 11.3 Run the nightly once manually before declaring it the gate; resolve any leaks or wake-ups it surfaces.
