...
`BuiltinSettingSpecs()` SHALL include the following keys in addition to the existing `editor.tab_size`, `editor.indent_width`, `editor.soft_tabs`, `editor.wrap`, `editor.colorscheme`, and `ui.scale`: `editor.font_family` (string), `editor.font_size` (int, range 8..32), `editor.line_endings` (enum: `lf`/`crlf`/`auto`), `editor.save.trim_trailing_whitespace` (bool), `editor.save.ensure_final_newline` (bool), `editor.format_on_save` (bool), `editor.autosave` (enum: `off`/`on_focus_change`/`after_delay`), `editor.autosave.delay_ms` (int, range 200..60000; idle debounce before an `after_delay` autosave), `editor.hover_delay_ms` (int, range 0..2000), `ui.layout_mode` (enum: `auto`/`regular`/`compact`), `ui.layout_compact_breakpoint_px` (int, range 600..2000), `ui.scrollbar_size` (enum: `compact`/`regular`/`large`), `ui.resize_handle_size` (enum: `compact`/`regular`/`large`), `ui.show_status_bar` (bool), `terminal.shell` (string), `terminal.font_size` (int, range 8..32), `terminal.font_family` (string), `diagnostics.min_severity` (enum: `hint`/`info`/`warning`/`error`).
